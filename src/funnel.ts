// Interactive funnel diagram — recreates the screenshot reference (Traffic → Home → Search/Browsing → PDP → Cart → Checkout)
// with red leak arrows at each stage. Hovering a stage or transition highlights it and surfaces details.
...
type Transition = {
  ...
  to: string;
  pct: number;
  /** Optional label override placed at the arrow midpoint */
  labelOverride?: string;
  /** Curve control — drawn as a quadratic Bézier when curve != null */
  curve?: { cx: number; cy: number };
  /** Visual treatment */
  variant: 'forward' | 'shortcut' | 'leak';
};
...
const LEAKS: Leak[] = [
  { id: 'leak-traffic',  fromStageId: 'traffic',  label: 'bounce',         x: 120 },
  { id: 'leak-home',     fromStageId: 'home',     label: 'home exit',      x: 320 },
  { id: 'leak-search',   fromStageId: 'search',   label: 'no results',     x: 510 },
  { id: 'leak-browsing', fromStageId: 'browsing', label: 'no results',     x: 510 },
  { id: 'leak-pdp',      fromStageId: 'pdp',      label: 'exit',           x: 750 },
  { id: 'leak-cart',     fromStageId: 'cart',     label: 'abandoned cart', x: 980 },
  { id: 'leak-checkout', fromStageId: 'checkout', label: 'returns',        x: 1190 },
];
...
function transitionPath(t: Transition): { d: string; midX: number; midY: number } {
  ...
  if (t.curve) {
    ...
    // Approximate midpoint on a quadratic Bézier at t=0.5
    const midX = 0.25 * x1 + 0.5 * t.curve.cx + 0.25 * x2;
    const midY = 0.25 * y1 + 0.5 * t.curve.cy + 0.25 * y2;
    return { d, midX, midY };
  }

  // Smooth cubic for normal transitions
  const dx = (x2 - x1) * 0.5;
...
export function renderFunnel(root: HTMLElement): void {
  ...
  svg.setAttribute('aria-label', 'E-commerce funnel diagram showing stages and leakage at each step');
  ...
  // ---- Leak arrows (drawn first so they sit behind) ----
  ...
  LEAKS.forEach((leak) => {
    ...
    const labelX = stage.x + STAGE_W / 2;
  ...
  TRANSITIONS.forEach((t) => {
    ...
    labelBg.setAttribute('stroke-width', isShortcut ? '1' : '1');
  ...
  STAGES.forEach((stage) => {
    ...
    rect.style.transition = 'transform 180ms ease, filter 180ms ease';
  ...
  // ---- Leak row banner ----
  const banner = document.createElementNS(svgNS, 'g');
  banner.setAttribute('transform', 'translate(60, 595)');

  // Render the SVG into the host
  root.innerHTML = '';

  // Outer wrapper to give the diagram a card background + caption
  const wrapper = document.createElement('div');
  wrapper.className = 'relative overflow-hidden rounded-3xl bg-zinc-card p-6 sm:p-10';
  wrapper.style.backgroundImage =
    'radial-gradient(ellipse 60% 50% at 50% 0%, rgba(250, 225, 148, 0.05) 0%, transparent 70%)';

  const svgHost = document.createElement('div');
  svgHost.className = 'overflow-x-auto';
  svgHost.appendChild(svg);
  wrapper.appendChild(svgHost);

  // Leak row legend + caption
  const legend = document.createElement('div');
  legend.className =
    'mt-6 rounded-2xl border border-[#e87b6b]/30 bg-[#e87b6b]/[0.06] p-5 text-sm';
  legend.innerHTML = `
    <div class="font-medium text-[#f3a99e]">Leak row — lost revenue at every stage</div>
    <p class="mt-1 text-[#e8b4ab]/80">
      Each red arrow represents revenue that left the funnel before reaching checkout.
    </p>
  `;
  wrapper.appendChild(legend);

  const caption = document.createElement('p');
  caption.className = 'mt-4 text-sm text-neutral-500';
  caption.textContent =
    'Percentages reflect typical e-commerce benchmarks. Real numbers come from each merchant’s GA4 + store analytics.';
  ...
  function highlightStage(stageId: string) {
    edgeGroup.querySelectorAll<SVGPathElement>('[data-transition-id]').forEach((p) => {
      ...
      p.setAttribute('opacity', involved ? '1' : '0.2');
    ...
    stageGroup.querySelectorAll<SVGGElement>('[data-stage-id]').forEach((g) => {
      ...
      (g.firstChild as SVGRectElement).style.filter = dimmed ? 'saturate(0.3) brightness(0.7)' : 'none';
    });
  }
  ...
  function clearHighlight() {
    edgeGroup.querySelectorAll<SVGPathElement>('[data-transition-id]').forEach((p) => {
      ...
      p.setAttribute('stroke-width', t.variant === 'shortcut' ? '2' : '2');
    });
    leakGroup.querySelectorAll<SVGPathElement>('[data-leak-id]').forEach((p) => {
